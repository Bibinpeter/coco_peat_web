...
export const ProductGrid: React.FC<ProductGridProps> = ({ onOpenSpecs, onOpenQuote, onCursorChange }) => {
  ...
  return (
    <section id="products" className="py-24 bg-[#090a0d] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredProducts.map((product) => (
            <div
              key={product.id}
              className="spotlight-card border border-white/10 p-6 group cursor-pointer hover:border-emerald-500/40 transition-all duration-300 flex flex-col justify-between"
              onMouseEnter={() => {
                sound.playHover();
            ...
            >
              <div>
                ...
                <div className="relative rounded-xl overflow-hidden aspect-[16/10] bg-space-950 mb-6 border border-white/5">
                  <img
                    src={product.image}
                    alt={product.name}
                  ...
                  <div className="absolute top-4 right-4 flex flex-wrap gap-1.5 justify-end">
                    {product.tags.map((tag, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 rounded bg-space-900/90 border border-white/10 font-mono text-[10px] text-gray-300 backdrop-blur-md uppercase"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  {/* Expansion Badge */}
                  <div className="absolute bottom-4 left-4 bg-emerald-500/90 text-black backdrop-blur-md px-3 py-1 rounded-md font-mono text-xs font-bold uppercase">
                    {product.expansion}
                  </div>
                </div>
              ...
              <div className="border-t border-white/10 pt-4 mt-4 flex items-center justify-between font-mono text-xs">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-space-800 border border-white/15 text-white font-bold hover:bg-emerald-500 hover:text-black hover:border-emerald-400 transition-all flex items-center gap-1.5"
                >
        ...
        <div className="mt-16 p-6 rounded-2xl glass-panel border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-coir-amber/20 border border-coir-amber/40 flex items-center justify-center text-coir-gold">
              <ShieldCheck className="w-6 h-6" />
            </div>
            <div>
              <h4 className="text-lg font-bold text-white">Need Custom EC, Mesh Size or Buffer Ratio?</h4>
              <p className="text-xs text-gray-400 font-mono">Our R&amp;D lab custom-blends coir ratios for specific greenhouse crops (Tomatoes, Cannabis, Strawberries, Cut Flowers).</p>
            </div>
          </div>
          <button
            onClick={() => {
            ...
            className="whitespace-nowrap px-6 py-3 rounded-xl bg-space-850 border border-white/15 text-emerald-400 font-mono text-xs font-bold uppercase hover:bg-emerald-500/10 transition-all"
          >
